...
#  str_view(), str_view_all() 함수  #view_all 은 다 나옴
...
str_locate(mysentence, "ing") #결과 list로 나옴
...
str_replace(mysentence, "ing", "ING") #sub랑 똑같은거
str_replace_all(mysentence, "ing", "ING") # 다바꿔주는거
...
str_sub(mysentence, 1, 10) #substring 과 같은것
str_sub(mysentence2, 1, 10)

# str_length() 함수 

str_length(mysentence) #글자의 수를 세는 
...
mypattern1 <- gregexpr("ing", mysentence2)#위치를 잡아주는 것 mypattern1
mypattern1
...
str_see <- function(rx) str_view_all(mystring, rx) #위아래 똑같음
...
grep("ab[c-e]", strings, value=TRUE) #c부터 e 까지 다됨
grep("ab[^c]", strings, value=TRUE) #c를 뺀놈 [^]는 c 뺸거
grep("(a|b)c", strings, value=TRUE) #c가있는데 a혹은 b가 있다.
grep("^ab", strings, value=TRUE) # 앵커가 되니까 시작이 ab로 시작하는거
grep("\\^ab", strings, value=TRUE) #
...
R_wiki_sent <- strsplit(R_wiki_para[[1]], split="\\.") #벡터인데 결과가 리스트이니까 결과값이 안나온거?
R_wiki_sent

R_wiki_word <- list(length(R_wiki_sent))
for(i in 1:length(R_wiki_sent)){
  R_wiki_word[[i]] <- strsplit(R_wiki_sent[[i]], split=" ") #word sentence 가 벡터가 되야해서 그냥 할 수 없음 몇개로 쪼개질지 모르니까 for해서 로프하는게 좋다
} # wiki_sent 2개 sentence가 몇개일지는 모르겠지만 vecter 로 만들어진 놈을 split 으로 끊어라
R_wiki_word
R_wiki_word[[1]][[2]][4] #첫번째 list의 두번째 list의 4번째 element

# 문장 단위에서 list 형식을 vector 형식으로 변환 

R_wiki_sent #2개의 list 각각 list에 3개의 string element 가 있는 형태
