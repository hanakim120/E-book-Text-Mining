...
mytext #하나의 element 가 있는 커랙터 벡터
mytext_c <- VCorpus(VectorSource(mytext)) #corpus형태로 바꿈
mytext_c
summary(mytext_c)
mytext_c[[1]]
mytext_c[[1]]$content
...
#  특수문자의 확인 
myfunc <- function(x){str_extract_all(x, "[[:punct:]]{1}")} #대괄호는 메타커랙터, 안에는 커랙터클래스고 puntion이 1개있는 놈을 몽땅찾아라
#이걸 myfunc으로 만들었음
mypuncts <- lapply(mypaper, myfunc) #mypaper에 myfunc을 적용해라, 워닝은 상관없음, lapply를 list에서 많이씀
table(unlist(mypuncts)) #table은 list를 잡지않음 unlist해서 벡터로 만들고 table구하면 나옴

# 특수문자 사용 전후에 단어를 함께 확인하기 
myfunc <- function(x) {str_extract_all(x, "[[:alnum:]]{1,}[[:punct:]]{1}[[:alnum:]]{1,}")} # 컴마앞에 뭔가 알파뉴메릭(숫자,영문자등)1개이상
#뒤에도 알파뉴메릭1개이상인거 찾아라
mypuncts <- lapply(mypaper, myfunc)
table(unlist(mypuncts)) #컴마랑 점은 안나왔다 뒤에 1을 나왔으니까. 0넣으면 다 나올것임.


myfunc <- function(x) {str_extract_all(x, "[[:alnum:]]{1,}[[:punct:]]{1}[[:alnum:]]{0,}")}
mypuncts <- lapply(mypaper, myfunc)
...
table(unlist(mydigits)) #숫자가 의미가있나 없나 알아볼수있음

#  대문자 확인 
myfunc <- function(x) {str_extract_all(x, "[[:upper:]]{1}[[:alpha:]]{1,}")} #대분자한개있고 영문자1개이상인놈들찾아라
...
dtm[1:5, 1:10] #list임 
inspect(dtm) #실제로 보려면 inspect로 봐야함
